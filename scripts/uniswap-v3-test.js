...
//            console.log('=== blockNumber: ===', events[i].blockNumber);
            ...
            try {
                switch (events[i].eventName) {
                    ...
                    case 'Mint':
                        ...
                        await mint(eventData.owner, eventData.tickLower, eventData.tickUpper, eventData.amount0, eventData.amount1, eventData.amount);
                        break;
                    case 'Burn':
                        if (swapEventDatas.length > 0) {
                            await batchSwap(swapEventDatas);
                        ...
                        await burnCatch(eventData.owner, eventData.tickLower, eventData.tickUpper, eventData.amount, eventData.amount0, eventData.amount1);
                        break;
                    case 'Collect':
                        if (swapEventDatas.length > 0) {
                            await batchSwap(swapEventDatas);
                        ...
                        await collect(eventData.owner, eventData.tickLower, eventData.tickUpper, eventData.amount0, eventData.amount1, eventData.recipient);
                        break;
                    default:
    ...
    async function burnCatch(ownerAddress, tickLower, tickUpper, amount, amount0, amount1) {
