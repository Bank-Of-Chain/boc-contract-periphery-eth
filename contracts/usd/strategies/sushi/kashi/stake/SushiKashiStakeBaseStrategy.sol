...
abstract contract SushiKashiStakeBaseStrategy is BaseClaimableStrategy {
    ...
    function getPoolId() virtual public pure returns (uint16);
    function getKashiPair() virtual public pure returns (IKashiPair);
    ...
    function claimRewards() internal override returns (address[] memory _rewardsTokens, uint256[] memory _claimAmounts) {
        getKashiPair().accrue();
    ...
    function depositTo3rdPool(address[] memory _assets, uint256[] memory _amounts) internal override {
        ...
        _bentoBox.setMasterContractApproval(
            // approval info
            address(this), _kashiPair.masterContract(), true,
            // empty signature
        ...
        IERC20Upgradeable(_asset).safeApprove(
            address(_bentoBox), 0);
        IERC20Upgradeable(_asset).safeApprove(
            address(_bentoBox), _amount);
        (, uint256 shareOut) = _bentoBox.deposit(
            _asset, address(this), address(this), _amount, 0);
    ...
    function withdrawFrom3rdPool(uint256 _withdrawShares, uint256 _totalShares) internal override {
        ...
        if (pairLeft < KASHI_MINIMUM) {
            ...
            console.log('[%s] kashi totalSupply will be less than minimum, so cut the withdrawn lp amount');
        }
