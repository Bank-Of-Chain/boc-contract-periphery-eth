...
abstract contract ETHBaseClaimableStrategy is ETHBaseStrategy {
    ...
    function repay(uint256 _repayShares, uint256 _totalShares) public virtual override onlyVault returns (address[] memory _assets, uint256[] memory _amounts) {
        // if withdraw all need claim rewards
        if (_repayShares == _totalShares) {
            (bool claimIsWorth, address[] memory assets, uint256[] memory amounts) = claimRewards();
            if (claimIsWorth) {
